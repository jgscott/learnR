...
Data files:  
* [trafficdeaths.csv](trafficdeaths.csv): Year by year state fatalities due to traffic accidents, along with some potential predictors.  These states are labeled by something called a FIPS code, for which you'll need...  
...
head(traffic2)
```
Now the traffic-fatalities data will be accompanied by helpful labels, rather than FIPS codes.  
...
relationship between fatality rate (response variable) and miles per driver (predictor).  What do you see?  
3) Make a lattice plot that stratifies the scatter plot of fatality rate versus miles driven by the categorical variable indicating whether the state has mandatory jail for drunk driving. What does this plot suggest?  
